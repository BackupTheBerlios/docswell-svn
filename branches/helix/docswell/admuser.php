...
// Check if there was a submission
  while (is_array($HTTP_POST_VARS) && list($key, $val) = each($HTTP_POST_VARS)) {
	...
	if (empty($username) || empty($password) || empty($email_usr)) { // Do we have all necessary data?
	  ...
	  $query = "update DOKUMENT set ANGELEGTVON='$username' where ANGELEGTVON='$old_username'";
	  $db->query($query);
	  $query = "update KOMMENTAR set AUTOR='$username' where AUTOR='$old_username'";
	  $db->query($query);
        }
	...
	break;
      ...
      case "u_kill":
        ...
        $query = "update DOKUMENT set ANGELEGTVON='$anonymous' where ANGELEGTVON='$old_username'";
        $db->query($query);
	$query = "update KOMMENTAR set AUTOR='$anonymous' where AUTOR='$old_username'";
	$db->query($query);
